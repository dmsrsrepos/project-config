 ...
 */
// Meta info
import { defineConfigObject, defineConfigs, useCommand as useInternalCommand, useCommands as useInternalCommands, useLogger as useInternalLogger, useOutputChannel as useInternalOutputChannel, useStatusBarItem, useDisposable, } from 'reactive-vscode';
 ...
 */
export const useCommand = (commandFullKey: Command, callback: (...args: any[]) => any): void => useInternalCommand(commandFullKey, callback);
/**
 * Register multiple commands. See `vscode::commands.registerCommand`.
 */
export const useCommands = (commands: Partial<Record<Command, (...args: any[]) => any>>): void => useInternalCommands(commands);
/**
 ...
 */
export const useLogger = (loggerName: LoggerName = displayName ?? name ?? extensionId, getPrefix?: ((type: string) => string) | null) => useInternalLogger(loggerName, { 'getPrefix': getPrefix });
/**
 * @reactive `window.createOutputChannel`
 */
export const useOutputChannel = (outputName: LoggerName = displayName ?? name ?? extensionId) => useInternalOutputChannel(outputName);
/**
 ...
 */
export const useConfig = memo(<K extends ConfigurationSection>(section: K) => defineConfigs<typeof projectKitDefaults[K]>(section, projectKitDefaults[section]));
/**
 * Define configurations of an extension. See `vscode::workspace.getConfiguration`.
 */
export const useConfigObject = memo(<K extends ConfigurationSection>(section: K) => defineConfigObject<typeof projectKitDefaults[K]>(section, projectKitDefaults[section]));
/**
